  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-5xl px-4 pb-28 pt-8 text-stone-900 transition-colors dark:text-stone-100 sm:px-6 sm:pt-12">
      <header className="mb-8 flex items-start justify-between">
        <div>
          <p className="text-sm text-stone-500 dark:text-stone-400">Our schedule</p>
          <h1 className="text-3xl font-semibold tracking-tight text-stone-900 dark:text-stone-100">Plans</h1>
        </div>

        <div className="flex items-center gap-2">
          <ThemeToggle />

          <button type="button" aria-label="Add plan" className="flex h-11 w-11 items-center justify-center rounded-full bg-emerald-900 text-white transition hover:bg-emerald-800 dark:bg-emerald-700 dark:hover:bg-emerald-600">
            <Plus size={21} />
          </button>
        </div>
      </header>
      ...
      <section className="mb-6 rounded-3xl bg-emerald-900 p-6 text-white shadow-sm dark:bg-emerald-950 dark:ring-1 dark:ring-emerald-700/30">
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-emerald-100 dark:text-emerald-300">Coming up</p>
            <p className="text-xl font-semibold">
              {loading ? "Loading..." : `${events.length} upcoming ${events.length === 1 ? "event" : "events"}`}
            </p>
          </div>
        </div>
      </section>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-stone-900 dark:text-stone-100">Upcoming</h2>
        ...
        <div className="overflow-hidden rounded-3xl border border-stone-200 bg-white dark:border-white/[0.07] dark:bg-white/[0.035] dark:ring-1 dark:ring-white/[0.025]">
          {loading && (
            <div className="p-5 text-sm text-stone-500 dark:text-stone-400">Loading calendar...</div>
          )}
          ...
          {!loading &&
            ...
            events.map((event, index) => (
              <button
                ...
                className={`flex w-full items-center gap-4 p-4 text-left transition hover:bg-stone-50 dark:hover:bg-white/[0.04] ${index !== events.length - 1 ? "border-b border-stone-100 dark:border-white/[0.06]" : ""}`}
              >
